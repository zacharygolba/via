...
impl<'a, T: Default> Location<'a, T> {
    pub fn at(&mut self, pattern: &'static str) -> Location<T> {
        let mut labels = Labels::parse(pattern);
        Location(self.0.insert(&mut labels))
    }
}
...
impl<T: Default> Router<T> {
    ...
    pub fn visit<'a, 'b>(&'a self, pattern: &'b str) -> Visit<'a, 'b, T> {
        Visit {
            node: &self.0,
